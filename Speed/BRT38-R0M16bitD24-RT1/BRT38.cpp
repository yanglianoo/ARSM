...
bool BRT38::Init(std::string &configPath)
{
    std::cout<<"BRT38 Speed Sensor"<<std::endl;
...
int BRT38::GetFrameData(std::vector<DataBase *> &data)
{
    if(data.size() <= 1 )
    {
...
int BRT38::CloseDevice()
{
    if (serial_fd_ != -1)
    {
        std::cout<<"BRT38 close" << std::endl;
        close(serial_fd_);
        serial_fd_ = -1;
    }   
    return 0;
